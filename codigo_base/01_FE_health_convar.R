...
AgregarVariables <- function(dataset) {
 ...
 # ------------------------------------------------------------------------
  # VARIABLES MANUALES AVANZADAS - HEALTH ECONOMICS
  # ------------------------------------------------------------------------
  ...
  if(PARAMS$feature_engineering$param$variablesmanuales) {
    ...
    dataset[, elderly_dependency_adjusted := SP.POP.DPND.OL * SP.POP.65UP.TO.ZS]

    dataset[, aging_speed := SP.POP.65UP.TO.ZS - shift(SP.POP.65UP.TO.ZS, 5),
            by = Country Code]

    dataset[, inflation_3y := FP.CPI.TOTL.ZG +
                               shift(FP.CPI.TOTL.ZG,1) +
                               shift(FP.CPI.TOTL.ZG,2),
            by = Country Code]

    dataset[, inflation_volatility := sd(FP.CPI.TOTL.ZG, na.rm = TRUE),
            by = Country Code]

    dataset[, reserve_crisis := as.integer(FI.RES.TOTL.MO < 3)]

    dataset[, credit_boom := FD.AST.PRVT.GD.ZS - shift(FD.AST.PRVT.GD.ZS,3),
            by = Country Code]

    dataset[, env_intensity := EN.GHG.CO2.PC.CE.AR5 / (NY.GDP.PCAP.KD + 1)]

    dataset[, urban_pressure := EN.POP.DNST * SP.URB.TOTL.IN.ZS]

    dataset[, water_stress_health := ER.H2O.INTR.PC * EN.POP.DNST]

    dataset[, health_mch_index := (SH.DTH.IMRT + SH.DTH.MORT + SH.STA.MMRT) / 3]

    dataset[, immunization_coverage := (SH.IMM.IDPT + SH.IMM.MEAS) / 2]

    dataset[, mortality_gender_gap := SH.DYN.MORT.MA - SH.DYN.MORT.FE]

    dataset[, maternal_crisis := as.integer(SH.STA.MMRT > 300)]

    dataset[, real_consumption_power := NE.CON.PRVT.PC.KD / (FP.CPI.TOTL.ZG + 1)]

    dataset[, saving_capacity := 1 - (NE.CON.TOTL.CD / (NY.GDP.MKTP.CD + 1))]

    dataset[, health_human_capital := (SE.SEC.ENRR + SE.TER.ENRR) / 2]

    dataset[, institutional_quality_health := SG.LAW.INDX * SE.TER.ENRR]

    dataset[, exchange_volatility :=
              abs(PA.NUS.ATLS - shift(PA.NUS.ATLS,1)) /
              (shift(PA.NUS.ATLS,1) + 1),
            by = Country Code]

    dataset[, exchange_depreciation_5y :=
              (PA.NUS.ATLS - shift(PA.NUS.ATLS,5)) /
              (shift(PA.NUS.ATLS,5) + 1),
            by = Country Code]

    dataset[, covid_dummy := as.integer(year %in% c(2020, 2021))]

    dataset[, covid_elderly_risk := covid_dummy * SP.POP.65UP.TO.ZS]

    dataset[, covid_consumption_shock :=
              ifelse(year %in% c(2020, 2021), NE.CON.PRVT.KD.ZG, 0)]

    dataset[, military_health_tradeoff :=
              MS.MIL.XPND.GD.ZS / (NE.CON.GOVT.ZS + 1)]

    dataset[, high_militarization := as.integer(MS.MIL.XPND.GD.ZS > 3)]

    dataset[, food_security_index :=
              AG.PRD.FOOD.XD / (EN.POP.DNST + 1)]

    dataset[, rural_income_proxy :=
              AG.YLD.CREL.KG * AG.LND.ARBL.HA.PC]
  }
